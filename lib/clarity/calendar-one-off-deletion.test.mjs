...
test("hard deletion rejects recurring series and preserves separate skip/cancel UI", () => {
  ...
  assert.match(calendarAgendaSource, /Cancel recurring series/);
...
test("completed and unresolved one-off records use the same deletion path", () => {
  ...
  assert.match(calendarAgendaSource, /readOnly && commitment\.recurrence === "none"/);
  assert.match(calendarAgendaSource, /commitment\.recurrence === "none"/);
});
