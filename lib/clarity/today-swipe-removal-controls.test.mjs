...
test("expanded proposed cards stay planning-focused except for passed-time reconciliation", () => {
  ...
  assert.match(
    proposedCardSource,
    /\{timePassed && \([\s\S]*Already done/,
  );
  assert.equal(
    [...proposedCardSource.matchAll(/Already done/g)].length,
    1,
  );
});
