...
test("expanded proposed cards expose removal alongside edit and completion", () => {
  assert.match(proposedCardSource, /Remove from plan/);
  ...
  assert.match(proposedCardSource, /Edit action/);
  assert.match(proposedCardSource, /Already done/);
});
...
test("proposed cards retain reliable accordion taps inside the swipe surface", () => {
  assert.match(proposedCardSource, /onClick=\{\(\) => onToggle\(action\.id\)\}/);
  assert.match(proposedCardSource, /aria-expanded=\{expanded\}/);
  assert.match(
    proposedPlanSource,
...
test("nested proposed-action controls remain independent of the card header", () => {
  const headerEnd = proposedCardSource.indexOf("</button>");
  assert.ok(headerEnd > -1);
  assert.ok(proposedCardSource.indexOf("Edit action") > headerEnd);
  assert.ok(proposedCardSource.indexOf("Already done") > headerEnd);
});
...
test("active action workspace exposes canonical removal only for active actions", () => {
  ...
  assert.match(actionWorkspaceSource, /action\.status === "active"/);
  ...
  assert.match(actionWorkspaceSource, /Change time/);
...
test("Calendar commitment deletion remains independently configured", () => {
  assert.match(calendarAgendaSource, /actionLabel="Delete"/);
  assert.match(calendarAgendaSource, /deleteCalendarCommitmentAction/);
