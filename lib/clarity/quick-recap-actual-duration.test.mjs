...
test("unplanned completed activity records optional actual duration through existing evidence storage", () => {
  assert.match(completedItem, /<ActionFields/);
  assert.match(completedItem, /actualMinutes: parsed\.data\.actualMinutes/);
  assert.match(actionFields, /<TimeSpentField/);
  assert.match(actionFields, /name="actualMinutes"/);
