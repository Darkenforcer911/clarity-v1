...
test("a three-day Catch Up continues into a valid current-day Today state", () => {
  const before = parseAuthoritativeReturnState({
    state: "catch_up",
    rangeStartDate: "2026-08-22",
    rangeEndDate: "2026-08-24",
    dayCount: 3,
  });
  const after = parseAuthoritativeReturnState({
    state: "ready_for_today",
    throughDate: "2026-08-24",
  });

  assert.equal(before.kind, "catch_up");
  assert.equal(before.dayCount, 3);
  assert.equal(after.kind, "ready_for_today");
  assert.deepEqual(
    resolveTodayGatewayPrimaryAction({
      currentDay: "Tuesday",
    ...
    {
      kind: "link",
      label: "Continue shaping Tuesday",
      href: "/today/plan",
      heading: "Your Tuesday plan is ready.",
      supportingText: "Review the proposed actions before beginning.",
    },
  );
});

test("authenticated shell links do not prefetch during a refreshed-session handoff", () => {
  assert.match(
    appShellSource,
    /href="\/today"\s+prefetch=\{false\}/,
  );
  assert.match(
    bottomNavigationSource,
    /href=\{item\.href\}\s+prefetch=\{false\}/,
  );
});
