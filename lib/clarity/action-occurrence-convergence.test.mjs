...
test("Weekly repeats on the canonical Action weekday and Custom owns selected weekdays", () => {
  assert.match(
    migration,
    /when 'weekly' then \(p_local_date - p_start_on\) % 7 = 0/i,
  );
  assert.match(
    migration,
    /v_routine_start := case[\s\S]*else p_local_date[\s\S]*end/i,
  );
  assert.match(
    migration,
    /p_recurrence_pattern = 'certain_days' then p_recurrence_days/i,
  );
});
...
test("Due remains on the Action and Calendar projects the same owner-scoped row", () => {
  ...
  assert.match(calendarAgenda, /title="Due"/);
...
test("the Action workspace prioritizes Done then the persistent Clarity entry", () => {
  ...
  const clarityPosition = actionDetail.indexOf("clarityHref={buildActionClarityHref");
