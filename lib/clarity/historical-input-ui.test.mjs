...
test("Quick Recap and Past Calendar share the completed-item form language", () => {
  ...
  assert.match(recapFormSource, /<ActionFields/);
  assert.match(recapFormSource, /mode="completed"/);
  assert.match(actionFieldsSource, /<Field label="What"/);
...
test("historical corrections use controlled buttons instead of raw radios", () => {
  ...
  assert.match(outcomeControlSource, /<HistoricalOutcomeCorrectionEditor/);
...
test("historical editors register with the shared bottom-navigation guard", () => {
  ...
  assert.match(outcomeControlSource, /useAppShellEditorState\(editing\)/);
  ...
  assert.match(appShellSource, /hidden=\{editorNavigationHidden\}/);
