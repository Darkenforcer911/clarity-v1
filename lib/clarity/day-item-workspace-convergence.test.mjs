...
test("full-screen Action and Calendar workspaces share control geometry", () => {
  assert.match(shell, /export function DayItemSecondaryActions/);
  assert.match(shell, /actions\.length > 1 \? "grid-cols-2" : "grid-cols-1"/);
  assert.match(shell, /export function DayItemMoreDisclosure/);
  assert.match(actionWorkspace, /<DayItemSecondaryActions>/);
  assert.match(actionWorkspace, /<DayItemMoreDisclosure/);
  assert.match(occurrenceWorkspace, /<DayItemSecondaryActions>/);
  assert.match(occurrenceWorkspace, /<DayItemMoreDisclosure/);
  assert.match(calendarDetail, /contained=\{false\}/);
});

test("Calendar occurrence removal matches the shared secondary button treatment", () => {
  assert.match(calendarDelete, /variant="outline"/);
  assert.match(calendarDelete, /className="h-11 w-full rounded-xl"/);
  assert.match(calendarDelete, /<Trash2 \/>/);
  assert.doesNotMatch(calendarDelete, /text-destructive hover:text-destructive/);
  assert.match(occurrenceWorkspace, /<Pencil \/>[\s\S]*Edit/);
  assert.match(occurrenceWorkspace, /<Trash2 \/>[\s\S]*>\s*Skip\s*</);
});

test("domain-specific workspace content follows the shared controls", () => {
  assert.ok(
    actionDetail.indexOf("<DayItemWorkspaceShell") <
      actionDetail.indexOf("<ActionLifeRelationships"),
  );
  assert.ok(
    occurrenceWorkspace.indexOf("<DayItemWorkspaceShell") <
      occurrenceWorkspace.indexOf("commitment.details"),
  );
});
...
test("one-off and repeating Action controls use accurate day-level language", () => {
  ...
  assert.match(actionWorkspace, /recurring \? "Skip" : "Remove"/);
  assert.doesNotMatch(actionWorkspace, /Skip today|Skip this occurrence|Remove from today|Remove this occurrence/);
