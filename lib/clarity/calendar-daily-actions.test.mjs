...
test("Calendar reads owned Daily Actions directly without commitment copies", () => {
  assert.match(calendarServiceSource, /\.from\("daily_plans"\)/);
  assert.match(
    calendarServiceSource,
    /\.eq\("user_id", authenticatedUserId\)[\s\S]*\.eq\("local_date", localDate\)/,
  );
  assert.match(calendarServiceSource, /daily_actions\(\*\)/);
  assert.doesNotMatch(
    calendarServiceSource,
