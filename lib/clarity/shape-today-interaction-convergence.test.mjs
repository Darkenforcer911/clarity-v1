...
test("the compact day-item controls use source-neutral primary labels", () => {
  ...
  assert.match(proposedAction, /action\.source_routine_id \? "Skip" : "Remove"/);
  assert.doesNotMatch(proposedAction, />\s*Edit action\s*</);
  assert.doesNotMatch(proposedAction, />\s*Remove from plan\s*</);

  assert.match(occurrenceWorkspace, />\s*Done\s*</);
  assert.match(occurrenceWorkspace, />\s*Edit\s*</);
  assert.match(occurrenceWorkspace, />\s*Skip\s*</);
  assert.match(occurrenceWorkspace, /triggerLabel="Remove"/);
