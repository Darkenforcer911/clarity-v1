...
const soFarSource = readFileSync(
...
test("attended is presented as Completed across Calendar outcome controls", () => {
  ...
  assert.match(soFarSource, /formatCalendarOutcomeLabel\(value\)/);
});
...
test("Calendar exposes general occurrence-level Correct outcome", () => {
  assert.match(agendaSource, />\s*Correct outcome\s*</);
  assert.match(agendaSource, /\["attended", "Completed"\]/);
  assert.match(agendaSource, /\["missed", "Missed"\]/);
  assert.match(agendaSource, /\["cancelled", "Cancelled"\]/);
