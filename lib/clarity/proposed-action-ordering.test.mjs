...
test("fixed-time Actions and Calendar commitments share one chronological presentation", () => {
  ...
  assert.match(dailyCommitments, /href=\{`\/calendar\?date=\$\{commitment\.occurrence_date\}&commitment=\$\{commitment\.id\}`\}/);
  assert.match(
    proposedPlan,
...
test("reorder mode collapses editors and uses only the compact card representation", () => {
  ...
  assert.match(
    proposedPlan,
    /function enterReorderMode[\s\S]*flushSync\(\(\) => \{[\s\S]*setExpandedActionId\(null\)[\s\S]*setReorderMode\(true\)/,
  );
  assert.match(
    proposedPlan,
...
test("ending or cancelling drag leaves collapsed resting rows with swipe restored", () => {
  ...
  assert.match(
    proposedPlan,
    /function completeDragCleanup[\s\S]*row\.style\.transform = ""[\s\S]*setExpandedActionId\(null\)[\s\S]*setDraggingActionId\(null\)/,
  );
  assert.match(
    proposedPlan,
    /enabled=\{!reorderMode\}/,
  ...
  assert.match(
    proposedPlan,
    /function exitReorderMode[\s\S]*setExpandedActionId\(null\)[\s\S]*setReorderMode\(false\)/,
  );
});
