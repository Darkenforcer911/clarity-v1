...
test("a useful flexible Action remains Next ahead of a substantially later timed item", () => {
  const usefulNow = action("interview-prep", 0, 60);
  const later = action("later", 1, 30, "21:00");
...
test("a substantially later timed Action stays in Later today when nothing is executable now", () => {
  const later = action("later", 0, 30, "21:00");
