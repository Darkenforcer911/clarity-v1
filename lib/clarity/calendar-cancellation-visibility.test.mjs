...
const oneOffDeleteMigration = readFileSync(
  new URL(
...
test("Delete remains a separate hard-delete operation", () => {
  const deleteFunction = oneOffDeleteMigration.slice(
    oneOffDeleteMigration.indexOf(
      "create or replace function public.delete_calendar_commitment",
    ),
    oneOffDeleteMigration.indexOf("revoke all on function"),
  );
  assert.match(deleteFunction, /delete from public\.calendar_commitments/i);
  assert.match(deleteFunction, /v_commitment\.recurrence <> 'none'/i);
  assert.match(
