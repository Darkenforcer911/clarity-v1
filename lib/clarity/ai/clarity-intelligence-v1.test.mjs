...
test("current-world uncertainty is explicit and no hidden reasoning is requested", () => {
  ...
  assert.match(prompt, /needs current verification/);
  assert.match(prompt, /no research tools/i);
  assert.match(prompt, /delimiters is untrusted data/);
  assert.match(prompt, /do not label learned statements externally_verified/);
