...
test("draft and sent images open the selected image in one full-screen viewer", () => {
  ...
  assert.match(ui, /width: item\.width,[\s\S]*height: item\.height/);
  assert.match(
    ui,
    /width=\{image\.width \?\? undefined\}[\s\S]*height=\{image\.height \?\? undefined\}/,
    "persisted image dimensions reserve message geometry when available",
  );
