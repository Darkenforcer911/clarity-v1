...
test("a long conversation gets one initial latest-message position", () => {
  ...
  assert.match(
    ui,
    /scroll\.scrollTop = clarityConversationBottom\(\{[\s\S]*clientHeight: scroll\.clientHeight,[\s\S]*scrollHeight: scroll\.scrollHeight/,
    "history writes the exact legal bottom",
  );
  assert.match(ui, /data-clarity-conversation-scroll/);
  assert.match(ui, /overflow-y-auto/);
  assert.match(
    ui,
    /useLayoutEffect\(\(\) => \{\s*if \(initialPositionedRef\.current\) return;\s*initialPositionedRef\.current = true;\s*scrollConversationToBottom\(\);/,
    "the initial bottom write happens in a layout effect before paint",
  );
});
...
test("mobile closed route entry is native flex layout with no readiness delay", () => {
  ...
  assert.doesNotMatch(
    ui,
    /initialConversationReady|initialMobileLayoutReady|max-md:invisible|document\.fonts|querySelectorAll\("img"\)|resolveClarityInitialConversationFrame/,
  );
  assert.match(
    ui,
