...
test("a long conversation gets one initial latest-message position", () => {
  ...
  assert.match(
    ui,
    /const expectedBottom = clarityConversationBottom\(\{[\s\S]*clientHeight: scroll\.clientHeight,[\s\S]*scrollHeight: scroll\.scrollHeight[\s\S]*scroll\.scrollTop = end\.offsetTop \+ end\.offsetHeight[\s\S]*Math\.abs\(scroll\.scrollTop - expectedBottom\) < 1/,
    "the actual history owner positions its end sentinel and verifies the legal bottom",
  );
  assert.match(ui, /data-clarity-conversation-scroll/);
  assert.match(ui, /overflow-y-auto/);
  assert.match(
    ui,
    /useLayoutEffect\(\(\) => \{\s*if \(initialPositionedRef\.current\) return;[\s\S]*const positionInitialHistory/,
    "the bounded initial bottom operation is owned by a layout effect",
  );
  assert.match(ui, /data-clarity-history-end/);
});

test("first mount and route re-entry each get a fresh pre-paint bottom write", () => {
  assert.match(
    ui,
    /export function ClarityConversation[\s\S]*const initialPositionedRef = useRef\(false\)/,
...
test("an initial zero range is rechecked once before history is exposed", () => {
  assert.equal(clarityConversationBottom({ clientHeight: 500, scrollHeight: 500 }), 0);
  assert.equal(clarityConversationBottom({ clientHeight: 500, scrollHeight: 600 }), 100);
  assert.match(
    ui,
    /if \(messages\.length === 0\) \{[\s\S]*positionInitialHistory\(\);[\s\S]*return;[\s\S]*requestAnimationFrame\(\(\) => \{[\s\S]*positionInitialHistory\(\)/,
    "existing history is not exposed based on a provisional zero range and gets one frame to establish overflow",
  );
  assert.match(
    ui,
    /initialHistoryReady \? "" : "max-md:invisible"/,
    "only history content is withheld during the bounded recheck",
  );
  assert.doesNotMatch(
    ui,
    /data-clarity-conversation-panel[\s\S]{0,400}max-md:invisible/,
    "page chrome and composer are never hidden by history initialization",
  );
});

test("mobile closed route entry is native flex layout with no readiness delay", () => {
  assert.match(
    clarityPage,
  ...
  assert.match(
    ui,
    /const fixedKeyboardPanel =[\s\S]*viewportLayout\.phase !== "closed"[\s\S]*fixedKeyboardPanel[\s\S]*\? `fixed/,
    "only OPEN and CLOSING leave normal flow",
  );
  assert.doesNotMatch(
    ui,
    /initialConversationReady|initialMobileLayoutReady|document\.fonts|querySelectorAll\("img"\)|resolveClarityInitialConversationFrame/,
  );
  assert.match(
    ui,
...
test("keyboard dismissal owns one visual transition before normal geometry", () => {
  ...
  assert.match(
    dismissalSource,
    /const hostRect = host\.getBoundingClientRect\(\);[\s\S]*resolveClarityClosedFlowTarget\([\s\S]*const target = keyboardDismissalTargetRef\.current;[\s\S]*height: target\?\.height[\s\S]*phase: "closing",[\s\S]*top: target\?\.top/,
    "closing targets the live normal-flow host slot instead of the stale pre-open panel rect",
  );
  assert.match(
    dismissalSource,
    /if \(!keyboardDismissalPendingRef\.current\)[\s\S]*keyboardDismissalTargetRef\.current = resolveClarityClosedFlowTarget[\s\S]*if \(current\.phase === "closing"\) return current/,
  ...
  assert.match(
    dismissalSource,
    /const closingTarget = keyboardDismissalTargetRef\.current;[\s\S]*Confirmation changes state only; closing already owns the geometry\.[\s\S]*height:[\s\S]*closingTarget\?\.height[\s\S]*phase: "closed",[\s\S]*top: closingTarget\?\.top/,
...
test("closing normalizes the live flow slot to the restored document baseline", () => {
  const liveHostDuringKeyboardDismissal = {
    currentDocumentScrollTop: 374,
    hostHeight: 522,
    hostTop: -170,
    restingDocumentScrollTop: 0,
  };
  const finalNativeClosedRect = { top: 204, height: 522 };

  assert.deepEqual(
    resolveClarityClosedFlowTarget(liveHostDuringKeyboardDismissal),
    finalNativeClosedRect,
    "the fixed transition ends at the same rect the flow host has after document restoration",
  );
  assert.match(
    ui,
    /const closingTarget = keyboardDismissalTargetRef\.current;[\s\S]*height:[\s\S]*closingTarget\?\.height[\s\S]*phase: "closed",[\s\S]*top: closingTarget\?\.top/,
  );
  assert.match(
    ui,
    /const fixedKeyboardPanel =[\s\S]*viewportLayout\.phase !== "closed"/,
    "the matching target is followed by removal of fixed positioning",
  );
});
...
test("keyboard close restores the navigation-bounded layout without accumulating offsets", () => {
  ...
  assert.match(
    ui,
    /fixedKeyboardPanel[\s\S]*viewportLayout\.phase !== "closed"/,
  );
...
test("normal history scrolling stays native outside keyboard dismissal", () => {
  ...
  assert.equal((ui.match(/scrollConversationToBottom\(\)/g) ?? []).length, 2);
  assert.match(
    ui,
...
test("keyboard changes resize geometry without moving conversation history", () => {
  ...
  assert.match(ui, /fixedKeyboardPanel[\s\S]*\? `fixed/);
});
