...
import {
  ...
  CLARITY_INITIAL_LAYOUT_STABLE_FRAMES,
...
test("a long conversation gets one initial latest-message position", () => {
  ...
  assert.match(
    ui,
    /history\.scrollTop = bottom;[\s\S]*positionedAtBottom: Math\.abs\(history\.scrollTop - bottom\) < 1/,
    "mobile history writes the exact bottom and verifies the resulting position",
  );
  assert.match(ui, /history\.querySelectorAll\("img"\)/);
  assert.match(ui, /document\.fonts\.status === "loaded"/);
...
test("first mount and route re-entry independently verify final bottom", () => {
  const snapshot = {
    headerBottom: 103,
    historyClientHeight: 410,
    historyScrollHeight: 4_500,
    hostTop: 204,
    navigationTop: 844,
    panelHeight: 632,
    panelTop: 204,
  ...
  const initialize = () => {
    const first = resolveClarityInitialConversationFrame({
      contentReady: true,
      current: snapshot,
    ...
    const second = resolveClarityInitialConversationFrame({
      contentReady: true,
      current: snapshot,
      positionedAtBottom: true,
      previous: snapshot,
      stableFrames: first.stableFrames,
    });
    const third = resolveClarityInitialConversationFrame({
      contentReady: true,
      current: snapshot,
      positionedAtBottom: true,
      previous: snapshot,
      stableFrames: second.stableFrames,
    });
    return [first, second, third];
  };

  assert.equal(CLARITY_INITIAL_LAYOUT_STABLE_FRAMES, 2);
  assert.deepEqual(initialize().map((frame) => frame.ready), [false, false, true]);
  assert.deepEqual(
    initialize().map((frame) => frame.ready),
    [false, false, true],
    "a remounted Clarity route gets a fresh readiness sequence",
  );
  assert.match(
    ui,
    /const initialPositionedRef = useRef\(false\)[\s\S]*initialConversationSnapshotRef[\s\S]*useRef<ClarityInitialConversationSnapshot \| null>\(null\)/,
  );
});
...
test("mobile route entry reveals only the final measured closed geometry", () => {
  ...
  assert.match(
    ui,
    /resolveClarityInitialConversationFrame\([\s\S]*frame\.ready[\s\S]*queueMicrotask\(\(\) => \{[\s\S]*setInitialConversationReady\(true\)/,
    "final geometry, content height, and bottom position are verified before reveal",
  );
  assert.match(
    ui,
    /initialConversationReady \? "" : "max-md:invisible"/,
    "neither provisional geometry nor top-of-history state is painted on mobile",
  );
  assert.match(
    ui,
  ...
  assert.equal(
    (ui.match(/setInitialConversationReady\(true\)/g) ?? []).length,
    1,
    "route entry has one visible geometry commit after readiness is verified",
  );
  assert.match(
    ui,
...
test("normal history scrolling stays native outside keyboard dismissal", () => {
  ...
  assert.equal(
    (ui.match(/history\.scrollTop = bottom/g) ?? []).length,
    1,
    "the only direct history position write is the verified pre-reveal bottom",
  );
