...
test("the composer dock is a persistent sibling of the scrollable history", () => {
  ...
  assert.match(
    ui,
    /data-clarity-conversation-scroll[\s\S]*data-clarity-composer-dock[\s\S]*bg-background[\s\S]*data-clarity-history-composer-gap[\s\S]*border-t border-border bg-background[\s\S]*bg-background pb-1/,
  );
  assert.doesNotMatch(
    ui,
    /data-clarity-composer-dock[\s\S]{0,800}bg-background\/95/,
  );
  assert.doesNotMatch(
    ui,
    /data-clarity-composer-dock[\s\S]{0,800}backdrop-blur/,
  );
...
test("composer keeps the complete opaque bordered shell while focused", () => {
  assert.match(
    ui,
    /data-clarity-composer-shell[\s\S]*className="relative min-w-0 rounded-2xl border border-border bg-card p-2 shadow-sm"/,
  );
