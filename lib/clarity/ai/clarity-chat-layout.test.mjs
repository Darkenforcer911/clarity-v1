...
test("a long conversation gets one initial latest-message position", () => {
  ...
  assert.match(
    ui,
    /if \(!initialPositionedRef\.current\)[\s\S]*!initialMobileLayoutReady \|\| viewportLayout\.height === null[\s\S]*return;[\s\S]*scrollConversationToBottom\(\);[\s\S]*queueMicrotask\(\(\) => setInitialConversationReady\(true\)\)/,
    "mobile history waits for final measured height, moves to latest, then reveals",
  );
...
test("mobile route entry reveals only the final measured closed geometry", () => {
  ...
  assert.equal(
    ...
    1,
    "route entry has one reveal commit after geometry and history are ready",
  );
  assert.match(
    ui,
