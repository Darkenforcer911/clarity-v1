...
test("the long textarea uses native internal overflow without touch interception", () => {
  assert.match(ui, /textarea\.style\.overflowY = next\.scrolls \? "auto" : "hidden"/);
  assert.doesNotMatch(ui, /touch-pan-y|overscroll-y-contain/);
  assert.doesNotMatch(ui, /touchmove|pointermove/i);
});
