...
test("Life route renders canonical objects with the owner-scoped derived projection", () => {
  assert.match(pageSource, /getLifePageModel\(\)/);
  assert.match(pageSource, /model=\{pageModel\.life\}/);
  assert.match(pageSource, /projection=\{pageModel\.projection\}/);
...
test("Life Areas own all canonical read-model groupings", () => {
  assert.match(viewSource, />\s*Active Life\s*</);
...
test("Life is read-first and routes meaningful changes through proposal plumbing", () => {
  ...
  assert.match(viewSource, /Tell Clarity what’s new/);
  assert.match(viewSource, /Talk to Clarity/);
  assert.doesNotMatch(viewSource, /Mentor/);
  assert.match(viewSource, /href=\{mentorLifeChangeHref\}/);
  assert.match(viewSource, /Current Direction/);
  assert.match(viewSource, /data-slot="life-projection"/);
  assert.match(viewSource, /What Clarity is still learning/);
});
...
test("Life Model V1 does not add intelligence or a parallel client store", () => {
  ...
  assert.doesNotMatch(viewSource, /confidence percentage|source_proposal_id|memory item id/i);
});
