...
test("proposed Actions do not imply completion before the day is active", () => {
  assert.match(
    proposedActionCard,
    /reorderControl \? "py-5 pr-5" : "p-5"/,
  );
  assert.doesNotMatch(proposedActionCard, /\bCheck\b/);
  assert.doesNotMatch(proposedActionCard, />\s*Kept\s*</);
  assert.doesNotMatch(
    proposedActionCard,
    /-ml-|translate-x-\[/,
  );
