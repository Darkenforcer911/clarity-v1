...
test("the shared Time control is primary, untimed by default, and removable", () => {
  assert.match(timeSelectorSource, />\{label\}<\/p>/);
  assert.doesNotMatch(actionFieldsSource, /Time \(optional\)/);
  assert.match(timeSelectorSource, /Add time/);
  ...
  assert.match(
    timeSelectorSource,
    /onChange\(""\);[\s\S]*setTimeVisible\(false\)/,
  );
...
test("Add Action renders Title, Time, Duration, Details, and Repeats in order", () => {
  ...
  const titlePosition = actionFieldsRender.indexOf("What needs to be done?");
  const timePosition = actionFieldsRender.indexOf("<OptionalActionTimeField");
  ...
  assert.ok(detailsPosition > durationPosition);
  assert.ok(repeatsPosition > detailsPosition);
