...
const completedEvidenceSource = readFileSync(
...
test("planned and completed Duration use one compact friendly input", () => {
  ...
  assert.match(timeSpentSource, /placeholder="30m or 1h 30m"/);
...
test("30m uses the shared minute value across Add, Edit, and completed flows", () => {
  assert.match(timeSpentSource, /placeholder="30m or 1h 30m"/);
  assert.match(timeSpentSource, /String\(result\.totalMinutes\)/);
  assert.match(actionFieldsSource, /name="estimatedMinutes"[\s\S]*value=\{durationMinutes\}/);
  assert.match(actionFieldsSource, /name="actualMinutes" value=\{value\}/);
  assert.match(addActionSource, /<ActionFields/);
  assert.match(proposedActionSource, /<ActionFields/);
  assert.ok((completedEvidenceSource.match(/<ActionFields/g) ?? []).length >= 2);
  assert.match(completedEvidenceSource, /mode="completed"/);
});

test("Due stays compact and reuses the contained shared Time row on narrow screens", () => {
  ...
  assert.match(dueField, /\{dueLocalDate && \([\s\S]*<TimeSelector/);
  assert.match(dueField, /label="Time"/);
  assert.match(dueField, /formatCommitmentTime\(dueLocalTime\) \?\? "No time"/);
  assert.doesNotMatch(dueField, /Optional time|type="time"/);
  assert.match(dueField, /w-full min-w-0 max-w-full/);
  assert.match(dueField, /\[min-inline-size:0\]/);
});

test("Due preserves date-only values and clears time independently", () => {
  const dueField = actionFieldsSource.slice(
    actionFieldsSource.indexOf("function ActionDueField"),
    actionFieldsSource.indexOf("export function DurationFields"),
  );
  ...
  assert.match(dueField, /onRemove=\{[\s\S]*dueLocalTime \? \(\) => onTimeChange\(""\) : undefined/);
  assert.match(
    dueField,
    /onClick=\{\(\) => \{[\s\S]*onDateChange\(""\);[\s\S]*onTimeChange\(""\);[\s\S]*Clear due/,
  );
  assert.doesNotMatch(dueField, /23:59|11:59|00:00|new Date\(/);
});
