...
import { getDayPeriod } from "./date-time.ts";
...
test("a late-night proposed plan is scoped to the rest of the current local day", () => {
  const melbourneLateNight = getDayPeriod(
    "Australia/Melbourne",
    new Date("2026-08-31T13:34:00.000Z"),
  );

  assert.equal(melbourneLateNight, "late_night");
  assert.deepEqual(
    resolve({ planStatus: "proposed", dayPeriod: melbourneLateNight }),
    {
      kind: "link",
      label: "Review Monday plan",
      href: "/today/plan",
      heading: "Your plan for the rest of Monday is ready.",
