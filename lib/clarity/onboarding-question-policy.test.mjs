...
test("question-policy rejection retains its safe focus diagnostics", () => {
  const state = {
    ...emptyState,
    unknowns: [
      {
        statement: "The user's current work situation remains unresolved.",
        materiality: "high",
      },
    ],
  };
  const policy = buildOnboardingQuestionPolicy({
    state,
    messages: [{ role: "user", content: "Work is uncertain right now." }],
    previousFocus: null,
  });
  ...
  assert.throws(
    () =>
      validateOnboardingQuestionSelection({
        ...
        policy,
        discovery: discovery({
          assistantMessage: "What would you want your life to look like next year?",
          questionFocus: {
            domain: "FUTURE_PULL",
            target: "the user's desired future",
            reason: "The desired future remains unclear.",
    ...
    (error) => {
      ...
      assert.equal(error.stage, "question_policy");
      assert.equal(error.code, "question_focus_not_allowed");
      assert.equal(error.safeMetadata.questionFocusDomain, "FUTURE_PULL");
      assert.equal(
        error.safeMetadata.policyReason,
