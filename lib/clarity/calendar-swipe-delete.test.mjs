...
test("explicit Calendar Delete reuses the existing deletion server action", () => {
  assert.match(
    calendarAgendaSource,
    /deleteCalendarCommitmentAction\([\s\S]*initialCalendarActionState,[\s\S]*formData/,
  );
  assert.match(calendarAgendaSource, /formData\.set\("commitmentId", commitment\.id\)/);
});

test("expanded cards keep Cancel but no longer show permanent Delete", () => {
  assert.match(calendarAgendaSource, /Cancel commitment/);
  assert.doesNotMatch(calendarAgendaSource, />\s*Delete commitment\s*</);
});
...
test("current and future commitments expose the canonical prefilled edit form", () => {
  ...
  assert.match(formSource, /commitment\?\.commitment_type \?\? "event"/);
...
test("Calendar commitments shown on Today are not swipe-delete cards", () => {
  assert.doesNotMatch(dailyCommitmentsSource, /SwipeToRemove/);
...
test("ordinary taps are not pointer-captured before a horizontal swipe exists", () => {
  ...
  assert.match(
    pointerMove,
    /if \(!gesture\.horizontal\) return;[\s\S]*setPointerCapture\(event\.pointerId\)/,
  );
});
