...
test("the Clarity route is an honest skeleton rather than a fake chatbot", () => {
  assert.match(page, /data-slot="clarity-conversation-skeleton"/);
  assert.match(page, />\s*Clarity\s*</);
  assert.match(
    page,
    /Think through what matters, explore your options, and work out what[\s\S]*to do next\./,
  );
  assert.match(page, /Your conversation with Clarity will live here\./);
  assert.match(page, /Conversation isn’t connected in this build yet\./);
  assert.doesNotMatch(page, /<form|<input|<textarea|New chat|OpenAI|Anthropic|Gemini/);
});
...
test("memory distinguishes raw user statements from canonical or verified truth", () => {
  ...
  assert.match(memory, /One continuous conversation archive/);
...
test("architecture defers persistence and prohibits cross-user raw context", () => {
  assert.match(architecture, /one-conversation-per-user/);
  assert.match(architecture, /append-only messages/);
  ...
  assert.match(architecture, /No research integration exists in V1/);
