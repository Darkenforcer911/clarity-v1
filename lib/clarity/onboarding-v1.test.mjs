...
test("onboarding voice avoids polished AI cadence without weakening reasoning", () => {
  ...
  assert.match(systemPrompt, /observation adds a useful inference/i);
  assert.match(systemPrompt, /Never paraphrase the user's answer merely to prove you listened/i);
  assert.match(systemPrompt, /1–2 sentences, usually under about 35 visible words/i);
  assert.match(systemPrompt, /simple vocabulary, short sentence structure/i);
  assert.match(systemPrompt, /Keep sophisticated reasoning internal/i);
  assert.match(systemPrompt, /Speak simply even when the internal reasoning is complex/i);
