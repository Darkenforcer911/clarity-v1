...
test("question selection, insight, route expansion, and stopping policy are prompt-level contracts", () => {
  ...
  assert.match(prompt, /A branch can be well understood without becoming the person's priority/i);
  assert.match(prompt, /Current position describes what is true now/i);
  assert.match(prompt, /Active direction describes what the user is trying to do/i);
  assert.match(prompt, /Future pull describes the life or outcome they want/i);
  ...
  assert.match(prompt, /LOCATE establishes a consequential branch's identity, current state, and desired state/i);
  ...
  assert.match(prompt, /evidence about experience and capability, not proof of desired direction/i);
  assert.match(prompt, /Preserve an unknown target, outcome, or direction/i);
  assert.match(prompt, /Lightweight domain templates/i);
  assert.match(prompt, /semantic guidance, never required questionnaires or deterministic flows/i);
  assert.match(prompt, /target, applications, interviews, offers, and feedback/i);
  assert.match(prompt, /An unfamiliar branch must remain first-class/i);
  assert.match(prompt, /one or two follow-up questions/i);
  assert.match(prompt, /not a hard counter or forced rotation/i);
  assert.match(prompt, /Map enough of the person's consequential board before doing deep solution work/i);
  ...
  assert.match(prompt, /branch, missing dimension, materiality, why it matters, status/i);
  assert.match(prompt, /unresolved, bounded, resolved, or currently unanswerable/i);
  assert.match(prompt, /Never invent extra JSON fields/i);
