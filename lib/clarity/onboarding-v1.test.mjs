...
test("the compact prompt preserves question selection, truth, readiness, and stopping contracts", () => {
  ...
  assert.match(prompt, /identity, current state, desired state, stage, urgency, consequence/i);
  assert.match(prompt, /not every branch needs each one/i);
  ...
  assert.match(prompt, /DISCOVER → GROUND → LOCATE → COMPARE → DEEPEN → PRIORITISE → ACT/i);
  assert.match(prompt, /flexible loop, not a rigid wizard/i);
  assert.match(prompt, /Ambiguity comes before inference/i);
  assert.match(prompt, /surrounding context does not resolve it/i);
  assert.match(prompt, /smallest question needed to establish what it is/i);
  assert.match(prompt, /Do not assign unsupported identity, category, urgency, consequence/i);
  assert.match(prompt, /Do not clarify when the user's context already supplies that meaning/i);
  assert.match(prompt, /Broad statements often imply consequential unknowns/i);
  assert.match(prompt, /single unknown whose answer would most change/i);
  assert.match(prompt, /current priority, direction, next move, or interpretation/i);
  ...
  assert.match(prompt, /identity, state, desired state, and stage before detailed diagnosis/i);
  assert.match(prompt, /Past evidence may establish capability without establishing present direction/i);
  assert.match(prompt, /preserve the unknown rather than silently completing the branch/i);
  assert.match(prompt, /Unfamiliar branches remain first-class/i);
  assert.match(prompt, /avoid tunnel vision/i);
  assert.match(prompt, /do not remain artificially shallow/i);
  assert.match(prompt, /Newly introduced information may interrupt the current branch/i);
  assert.match(prompt, /potential urgency, consequence, dependency, or decision impact is higher/i);
  ...
  assert.match(prompt, /Preserve missing consequential prerequisites and assets as unknowns/i);
  assert.match(prompt, /high-materiality unknown/i);
  assert.match(prompt, /test contradictions before accepting a bottleneck/i);
  ...
  assert.match(prompt, /Reflection is optional/i);
  assert.match(prompt, /Never paraphrase merely to prove you listened/i);
  assert.match(prompt, /Think deeply and speak simply/i);
  ...
  assert.doesNotMatch(
    ...
    /\b(?:Ahmed|Meli|Yusuf|L1|L2|nursing|barbering|forex|paperwork|visa)\b/i,
  );
...
test("golden personas cover contrasting evidence and possibility patterns", () => {
  assert.deepEqual(
    ...
    new Set([
      ...
      "stable-worker-considering-career-change",
      ...
      "vague-unfamiliar-obligation-needs-grounding",
      "context-grounded-obligation-skips-redundant-clarification",
      "verbal-economy-current-work",
      "verbal-economy-useful-reflection",
    ]),
  );
  assert.ok(
  ...
  const questionOnly = onboardingGoldenEvals.find(
    (item) => item.id === "verbal-economy-current-work",
  );
  assert.deepEqual(questionOnly?.expected.delivery, {
    preferredShape: "question_only",
    targetMaxVisibleWords: 35,
    reflectionMustAddDecisionValue: true,
  });
  assert.match(
    questionOnly?.expected.evidencePriority ?? "",
    /direct question.*locate the branch/i,
  );

  const usefulReflection = onboardingGoldenEvals.find(
    (item) => item.id === "verbal-economy-useful-reflection",
  );
  assert.deepEqual(usefulReflection?.expected.delivery, {
    preferredShape: "observation_then_question",
    targetMaxVisibleWords: 35,
    reflectionMustAddDecisionValue: true,
  });
  assert.ok(
    usefulReflection?.expected.recognizes.some((item) =>
      /not the immediate failure point/i.test(item),
    ),
  );
  ...
  const generalizedV1Cases = generalizedV1Ids.map((id) =>
    onboardingGoldenEvals.find((item) => item.id === id),
  );
  assert.ok(generalizedV1Cases.every(Boolean));
  assert.ok(
    generalizedV1Cases.every(
      (item) =>
        !item?.expected.responseMode.includes("SYNTHESIZE") &&
        item.expected.asksAtMostOneQuestion &&
        item.expected.preservesUncertainty,
    ),
  );
  assert.ok(
    generalizedV1Cases.every((item) =>
      item?.expected.avoids.some((avoidance) =>
        /assuming|mandatory|telling|treating|reducing|diagnosing|forcing|jumping|asking/i.test(
          avoidance,
        ),
      ),
    ),
  ...
  const vagueObligation = onboardingGoldenEvals.find(
    (item) => item.id === "vague-unfamiliar-obligation-needs-grounding",
  );
  assert.match(
    vagueObligation?.expected.evidencePriority ?? "",
    /establishes what the papers are for before triage/i,
  );
  assert.ok(
    vagueObligation?.expected.avoids.some((item) =>
      /deadline before establishing what/i.test(item),
    ),
  );

  const groundedObligation = onboardingGoldenEvals.find(
    (item) =>
      item.id === "context-grounded-obligation-skips-redundant-clarification",
  );
  assert.equal(groundedObligation?.expected.readiness?.action, "sufficient");
  assert.ok(
    groundedObligation?.expected.avoids.some((item) =>
      /context already establishes their identity/i.test(item),
    ),
  );
});
...
test("onboarding voice avoids polished AI cadence without weakening reasoning", () => {
  ...
  assert.match(systemPrompt, /Reflection is optional/);
  assert.match(systemPrompt, /Default to one short question/);
  assert.match(systemPrompt, /observation before it only when it adds a grounded inference/i);
  assert.match(systemPrompt, /Never paraphrase merely to prove you listened/i);
  assert.match(systemPrompt, /1–2 sentences, usually under about 35 visible words/i);
  assert.match(systemPrompt, /Think deeply and speak simply/i);
  assert.match(systemPrompt, /Avoid em dashes/);
  assert.match(systemPrompt, /professional or consultant narration, abstract framing/);
